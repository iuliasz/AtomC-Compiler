...
bool fnDef() {
    ...
    Type t;
    if (iTk->code == ID && iTk->next && iTk->next->code == ID && iTk->next->next &&
        iTk->next->next->code == LPAR) {
...
bool structDef() {
    ...
    if (consume(STRUCT)) {
        if (consume(ID)) {
            ...
            if (iTk->code == TYPE_INT || iTk->code == TYPE_DOUBLE || iTk->code == TYPE_CHAR ||
                iTk->code == STRUCT || iTk->code == RACC) {
